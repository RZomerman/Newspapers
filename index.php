...
function fetchPaper($prefix, $offset=0){
	...
	$pdffile = "/var/www/archive/" . $prefix . "_" . date('Ymd',strtotime("-" . $offset . " days")) . ".pdf";
	$pngfile = "/var/www/archive/" . $prefix . "_" . date('Ymd',strtotime("-" . $offset . " days")) . ".png";
	$pngcolorfile = "archive/color.png";
	$webfile = "/image.png";
	// $rootpath = getcwd() . "/";
	$rootpath = "/var/www/";
	...
	if (!file_exists($pngfile)){
		...
		if ($exists) {
			$command =  "convert -density 300 -background white -alpha remove '" . $pdffile .
				   "' -colorspace Gray -resize 825 '" . $pngfile . "'";
			exec($command, $output, $response);

			// Making the master color for browsers
			//$command =  "convert -density 300 -background white -alpha remove '" . $rootpath . $pdffile .
			//	   "' -resize 1080 -rotate 270 '" . $rootpath . $pngcolorfile . "'";
			//exec($command, $output, $response);
...
echo $imageresult;
debug_to_console($imageresult);

			// delete old image file (removing symbolic link)
			$command =  "rm /var/www/archive/image.png";
			// echo $command;
			exec($command, $output, $response);

			// create symbolic link to image.png for InkPlate10
			$command =  "cp $imageresult /var/www/archive/image.png";
			// echo $command;
			exec($command, $output, $response);

			// rotate the image to fit InkPlate 10 vertical
			$command =  "convert /var/www/archive/image.png -rotate 270 /var/www/archive/image.png";
			// echo $command;
			exec($command, $output, $response);


			// crop the image to fit InkPlate 10 vertical/horizontal
			$command =  "convert /var/www/archive/image.png -crop 1200x+0+0 /var/www/archive/image.png";
			// echo $command;
			exec($command, $output, $response);
