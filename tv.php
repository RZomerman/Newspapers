...
function fetchPaper($prefix, $offset=0){
	...
	$pngcolorfile = "archive/" . $prefix . "_" . date('Ymd',strtotime("-" . $offset . " days")) . "-color.png";
	$webfile = "/image.png";
	$rootpath = getcwd() . "/";
	// check if a jpg has already been created
	// if not we start checking for the PDF and converting
	if (!file_exists($pngcolorfile )){
		...
		if ($exists) {
			$command =  "convert -density 300 -background white -alpha remove '" . $rootpath . $pdffile .
				   "' -resize 3840x '" . $rootpath . $pngcolorfile . "'";
...
<?php if (strlen($imageresult) < 1) {
   ...
   echo "<img src='" . $imageresult . "' height='1080' class='paper' >";
 }
?>
</div>
</body>
